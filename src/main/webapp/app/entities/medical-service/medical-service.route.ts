...
export class MedicalServiceResolve implements Resolve<IMedicalService> {
  constructor(private service: MedicalServiceService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IMedicalService> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((medicalService: HttpResponse<MedicalService>) => {
          if (medicalService.body) {
            return of(medicalService.body);
          } else {
...
export const medicalServiceRoute: Routes = [
  {
    ...
    data: {
      ...
      pageTitle: 'hospitalManagementSystemApp.medicalService.home.title',
    },
